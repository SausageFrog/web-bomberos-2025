          ...
          {site.units.map((u, i) => (
            <Card key={i} className="border-0 shadow-md">
              ...
              <CardContent className="text-gray-700 space-y-1">
                {u.brand && u.model && (
                  <div><span className="font-semibold">Modelo: </span>{u.brand} {u.model}</div>
                )}
                {u.year && <div><span className="font-semibold">Año: </span>{u.year}</div>}
                {u.plate && <div><span className="font-semibold">Patente: </span>{u.plate}</div>}
                {u.notes && <p className="text-sm text-gray-600 pt-2">{u.notes}</p>}
                {u.features?.length ? (
                  <div className="pt-2">
